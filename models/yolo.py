...
# from Footfall_Detection.models.common import *
from Shelf_Empty_Detection.models.common import *
from Footfall_Detection.models.experimental import MixConv2d, CrossConv, C3
from Footfall_Detection.utils.general import check_anchor_order, make_divisible, check_file
from Footfall_Detection.utils.torch_utils import time_synchronized, fuse_conv_and_bn, model_info, scale_img, initialize_weights, select_device
...
class IDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True, single_special_cls=False):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
        ...
        self.single_special_cls = single_special_cls
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if self.single_special_cls: y[..., -1:].sigmoid_()
                if self.inplace:
...
class ASegment(IDetect):
    def __init__(self, nc=80, anchors=(), nm=32, npr=256, ch=(), inplace=True):
        super().__init__(nc, anchors, ch, inplace, True)
        self.nm = nm  # number of masks
        self.npr = npr  # number of protos
        self.no = 5 + 1 + nc + self.nm  # number of outputs per anchor
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect = IDetect.forward
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, IDetect, Segment, ISegment, ASegment, IRSegment)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, IDetect, Segment, ISegment, ASegment, IRSegment)):
            ...
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, ISegment, ASegment, IRSegment)) else self.forward(x)
